...
OLD_REPO_NAME="classical-and-quantum-gravity-research-article-latex-template"
...
sed "${SED_INPLACE[@]}" "s/$OLD_OWNER_NAME/$NEW_OWNER_NAME/g" cliff.toml README.md CITATION.cff .github/ISSUE_TEMPLATE/config.yml

find . -type d \( -name ".git" -o -name "__pycache__" -o -name ".venv" -o -name "node_modules" -o -name ".pytest_cache" \) -prune \
    -o -type f \( -name "*.py" -o -name "*.md" -o -name "*.toml" -o -name "*.yml" -o -name "*.cff" -o -name "LICENSE" \) \
